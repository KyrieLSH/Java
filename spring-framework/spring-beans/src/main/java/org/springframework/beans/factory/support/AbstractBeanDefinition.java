...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	 ...
	 * @throws BeanDefinitionValidationException in case of validation failure
	 * 在 Spring
	 * 置中存在 lookup-m thod replace method 两个配置功能，而这两个配置的加载其实就是将配置
	 * 统一存放在 BeanDefinition 中的 methodOverrides 属性里，这两个功能实现原理其实是在 bean
	 * 例化的时候如果检测到存在 method Overrides 属性，会动态地为当前 bean 生成代理并使用对应
	 * 的拦截器为 bea 做增强处理，相关逻辑实现在 bean 的实例化部分详细介绍
	 */
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		...
		if (count == 0) {
			...
			/**
			 * 标记 MethodOverride 'fg 米被被盖 避免参数类型检盒的开销
			 */
